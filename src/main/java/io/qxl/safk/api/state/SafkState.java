...
/**
 * SafkState -- Describes the status and values of an AFK player; or
 ...
 */
public record SafkState(SafkStatus status, int time, long timeout, long startTime, String reason)
{
	public static final SafkState DEFAULT = new SafkState(SafkStatus.INACTIVE, 129600, -1L, -1L, "");

	@Override
	public boolean equals(Object o)
	{
		if (o == this) { return true; }
		if (!(o instanceof SafkState s)) { return false; }
	...
	{
		return "SafkState{" + "status=" + this.status + ", time=" + this.time + ", timeout=" + this.timeout + ", startTime=" + this.startTime + ", reason=" + this.reason + '}';
	}
	...
	public SafkState ensureValid()
	{
		if (this.status() == SafkStatus.ACTIVE)
		{
			...
			return new SafkState(SafkStatus.ACTIVE, time, timeout, startTime, this.reason);
		}

		return this;
	}

	public SafkState copy()
	{
		return new SafkState(this.status(), this.time(), this.timeout(), this.startTime(), this.reason());
	}
}
